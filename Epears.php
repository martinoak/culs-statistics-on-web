...
<?php echo("<font color=red>you did not enter an integer between 3 and 30, correct</font>"); break;
  ...
  <form method=get> test level &nbsp;&nbsp; &alpha; = 0,05 
    ...
    <input type=integer size=1 name=n value="<?echo($n);?>">&nbsp;&nbsp;&nbsp;&nbsp;
    <input type=submit value="yes"> &nbsp; (enter an integer from 3 to 30)
    <br> random sample from normal distribution &nbsp;&nbsp; (X<sub>1</sub>,Y<sub>1</sub>),...,(X<sub><?php echo($n);?></sub>,Y<sub><?php echo($n);?></sub>) 
    <br> X<sub>1</sub>,...,X<sub><?php echo($n);?></sub>: &nbsp;&nbsp;&nbsp;&nbsp;
    <?php for ($i =0; $i <$n; $i++): ?>
    <input type=double name="x[]" size=1 value="<?echo($x[$i]);?>">
    <?php endfor;?> 
  ...
  <?php 
  if($so1*$so2==0):?> 
    <br><font color=red> at least one variance is equal to 0, this test cannot be used </font><?php 
  else:
    $r=$sxy/sqrt($so1)/sqrt($so2);
    if($r>=0.9999||$r<=-0.9999):?>
      <br><font color=red> correlation is equal to 1 or -1, this test cannot be used </font><?php   
    else:
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      hypothesis &nbsp;&nbsp; H<sub>0</sub>: &rho; = 0 &nbsp;&nbsp; is rejected <?php 
        else: ?> 
      |T| < t<sub><?php echo($sv)?></sub> <?php echo("(0.975)");?> 
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
      hypothesis &nbsp;&nbsp; H<sub>0</sub>: &rho; = 0 &nbsp;&nbsp; is not rejected<?php 
        endif;
    endif;
  endif;?>
