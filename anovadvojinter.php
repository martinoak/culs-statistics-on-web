...
<script>
function otev(){vzor = window.open("anovadvouinter.png","vzor","width=950, height=700");}
...
<td><br><h2>Dvojné třídění (ANOVA) s interakcemi:</h2></td>
...
hypotézu &nbsp;&nbsp; H<sub>0</sub> : &alpha;<sub>1</sub> = ... = &alpha;<sub><?php echo($ir); ?></sub> = 0  
  ...
  <?php 
    if($ffab>=$finvab): ?> 
      ...
      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;        
hypotézu &nbsp;&nbsp; H<sub>0</sub> : &gamma;<sub>1,1</sub> = ... = &gamma;<sub><?php echo($ir.",".$is); ?></sub> = 0  
         &nbsp;&nbsp;zamítneme
    <?php 
    else: ?> 
...
<br><br>
<?php
echo'<a href="anovadvoj.php?ir=',$ir,'&  is=',$is,'& ip=',$ip,'&';
for ($i =0; $i <$ir; $i++): 
for ($k =0; $k <$is; $k++): 
for ($j =0; $j <$ip; $j++): 
echo'x%5B%5D=',$x[$is*$ip*$i+$ip*$k+$j],'&';
endfor;endfor;endfor;
echo'a=2"> Dvojné třídění (ANOVA) bez interakcí </a>';
?>
